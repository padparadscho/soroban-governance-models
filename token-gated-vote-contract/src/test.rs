...
) -> TokenGatedContractClient<'a> {
    let contract_address = e.register(
        TokenGatedContract,
        TokenGatedContractArgs::__constructor(admin, token_address),
    );
    TokenGatedContractClient::new(e, &contract_address)
}
...
fn test_initialization() {
    ...
    let client = TokenGatedContractClient::new(&e, &contract_address);
